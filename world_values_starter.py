...
def main():
    ...
    # Get the indices of the nearest neighbors excluding the USA itself
    nearest_neighbors_indices = indices[0][1:]

    # Get the corresponding country names from the 'countries' list
    nearest_neighbors_countries = [countries[i] for i in nearest_neighbors_indices]

    # Print the 7 nearest neighbor countries of the USA
    print("USA's 7 Nearest Neighbors:")
    for country in nearest_neighbors_countries:
        print(country)
...
def _rmse_grid_search(training_features, training_labels, pipeline, parameters, technique):
   ...
   # Use GridSearchCV to create and fit a grid of search results
    grid = GridSearchCV(pipeline, parameters, scoring='neg_mean_squared_error', cv=5)
